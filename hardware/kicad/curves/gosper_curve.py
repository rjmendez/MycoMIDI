...
_TURN_DEGREES = 60.0
_TURN_RADIANS = math.radians(_TURN_DEGREES)
...
def _trace_gosper_points(order: int) -> list[tuple[float, float]]:
...
def gosper_curve_points(order: int) -> list[tuple[float, float]]:
    """Return normalized Gosper-curve points for a given recursion order.

    Unlike the orthogonal Hilbert curve's square-grid stair-steps, the Gosper
    curve has a more organic hexagonal/swirl boundary that better suits
    non-rectilinear decorative fills.
    """
...
def _max_expected_normalized_step(raw_points: list[tuple[float, float]]) -> float:
